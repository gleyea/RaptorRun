...
/*
	glGenTextures(1, OpenGL_ID);				// crée un "nom" de texture (un identifiant associé à la texture)
	glBindTexture(GL_TEXTURE_2D, OpenGL_ID[0]);	// et on active ce "nom" comme texture courante (définie plus bas)
	...
	if (isRGBA)
		gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA8, img_color->lenx, img_color->leny, GL_RGBA, GL_UNSIGNED_BYTE, img_all);
	else
		gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB8,  img_color->lenx, img_color->leny, GL_RGB,  GL_UNSIGNED_BYTE, img_color->data);
*/
//	delete[] img_all;
